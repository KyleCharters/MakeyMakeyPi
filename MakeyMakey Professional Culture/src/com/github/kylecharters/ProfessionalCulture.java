...
public class ProfessionalCulture implements KeyListener{
	public static void main(String[] args){
		new ProfessionalCulture();
	}
	
	public ProfessionalCulture(){
		loadSounds();
		window();
	}
	...
	public void loadSounds(){
		...
		any = TinySound.loadMusic(new File("Any.wav"));
		any.play(false);
		
		any2 = TinySound.loadMusic(new File("Any2.wav"));
		any2.play(false);
	...
	public void playRandom(int cat){
		...
		if(i == 1){
			any.play(false);
			System.out.println("Playing Any (1)");
			return;
		}else if(i == 2){
			any2.play(false);
			System.out.println("Playing Any (2)");
			return;
		}
		
		i = random.nextInt(catAmounts.get(cat));
		playSound(cat, i);
	}
